...
router.get('/:gtIdx', async (req, res) => {
    //url 에 세그먼트로 달려 오면 params로 아니면 req.query.변수명
    //console.log('=== gameTitleDetail 데이터 가져오기 ===');
    let gtIdx = req.params.gtIdx;
    //console.log(gtIdx);
    await dbPool.execute('call get_gameTitleDetail(?)', [gtIdx], function (err, results, fields) {
...
router.post('/', async (req, res) => {
    ...
    await dbPool.execute(gameTitleDetailCud, [0, gtIdx, gtdTitle, '', `${JSON.stringify(req.body)}`, 'post'], function (err, results) {
        //console.log(results[0]); // results contains rows returned by server
        //console.log(fields); // fields contains extra meta data about results, if available
        // If you execute same statement again, it will be picked from a LRU cache
        // which will save query preparation time and give better performance
...
router.delete('/:gtdIdx/:gtIdx', async (req, res) => {
    //console.log('=== gameTitle 데이터 가져오기 ===');
    //mysql2 공식문서를 보면 됨 https://sidorares.github.io/node-mysql2/docs
    let gtIdx = req.params.gtIdx;
    let gtdIdx = req.params.gtdIdx;
    await dbPool.execute(gameTitleDetailCud, [gtdIdx, gtIdx, '', '', `${JSON.stringify(req.params)}`, 'delete'], function (err, results) {
        //console.log(results[0]); // results contains rows returned by server
        //console.log(fields); // fields contains extra meta data about results, if available
        // If you execute same statement again, it will be picked from a LRU cache
        // which will save query preparation time and give better performance
