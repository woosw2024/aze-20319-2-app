...
router.get('/:gubun', async (req, res) => {
    ...
    await dbPool.execute(getGameTeam, [gtdIdx, teamCode, gubun], function (err, results) {
...
router.post('/', async (req, res) => {
    ...
    await dbPool.execute(setGameTeam, [gtdIdx, teamName, teamUserArray, bossFlagArray, arrayCount, '', `${JSON.stringify(req.body)}`, 'post'], function (err, results) {
        //console.log(results[0]); // results contains rows returned by server
        //console.log(fields); // fields contains extra meta data about results, if available
        // If you execute same statement again, it will be picked from a LRU cache
        // which will save query preparation time and give better performance
